...
"""
训练样本格式为[query, reply],并替换NSP 为SOP
"""
import os
...
def load_data(train_test='train'):
    ...
    with open(os.path.join(path, train_test, train_test + '.query.tsv')) as f:
        for l in f:
            span = l.strip().split('\t')
            q = span[1]
            #             if not tokenizer._is_punctuation(list(q)[-1]):
            #                 q += '。'
...
for line in words_data:
    q, rs = line[0], line[1:]
    for r in rs:
        pair_data.append([q, r])
data = pair_data
np.random.shuffle(data)
more_ids = batch_size - (len(data) % batch_size)
data = data + data[: more_ids]
